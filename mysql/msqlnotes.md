...
- 为什么使用存储过程？
1. 可以把处理封装起来，重复使用，简化操作。
2. 不用反复执行一系列步骤，保证数据完整性；进一步防止错误，保证数据一致性。
3. 简化对变动的管理。如果表名、列名或业务逻辑（或其他内容）有变化，只用改变存储过程，使用的人却不用知道变化；进一步保证安全性，限制对基础数据的访问减少数据讹误。
4. 提高性能。使用存储过程比使用单独SQL要快。

- 如何使用？
1. 创建存储过程
```sql
create procedure pcname([args])
begin
   存储体
end;
```
...
args是可选的，包括`IN`、`OUT`、`INOUT`类型的参数，分别是传入、传出、两者兼具的变量。

如果是在命令行创建，需要用DELIMITER //(除\以外的符号作为临时的语句结束分隔符) 来包括存储过程（对应end //，最后delimiter ;），原因是end后的`;`会混淆导致`SQL`语句结束。
...
显示支持的字符集列表：```show chatacter set```

显示支持的校对规则列表：```show collation```

可以对表和列分别指定字符集和校对

#### （第二十八章）安全管理
主要涉及和用户权限相关的，包括创建用户、赋予权限等。赋予用户特殊的权限进行访问控制和保护服务器。
